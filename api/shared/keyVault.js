...
async function getSecret(name) {
  ...
  try {
    ...
    if (!client) throw new Error(`KEY_VAULT_URL is not set`);
    ...
    const envMap = {
      ...
      'azure-storage-connection': process.env.AZURE_STORAGE_CONNECTION,
    };
    const val = envMap[name];
    if (val) { _cache[name] = val; return val; }
    throw new Error(`Secret '${name}' not found in Key Vault or environment. KEY_VAULT_URL=${KEY_VAULT_URL || 'NOT SET'} | ${err.message}`);
  }
}
