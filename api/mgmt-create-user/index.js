...
module.exports = wrapHandler('mgmt-create-user', async function (context, req) {
  ...
  if (!name || !email) {
    context.res = { status: 400, body: { error: 'Name and email address are required.' } }; return;
  }
  try {
    ...
    await sendEmail(email, 'Welcome to Hughes Shuttle Bus', welcomeTemplate(name, email, tempPassword));
    ...
    context.res = { status: 500, body: { error: 'Failed to create user. ' + err.message } };
    throw err;
  }
});
